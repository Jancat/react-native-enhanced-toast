...
class ToastContainer extends Component {
  ...
  _show = () => {
    ...
    if (!this._animating) {
      ...
      Animated.timing(this.state.opacity, {
        toValue: this.props.containerStyle.opacity || this.props.opacity,
  ...
  _onPressMessage = () => {
    const { onPressMessage, hideOnPress } = this.props
    if (onPressMessage) {
      onPressMessage()
      this._hide()
    } else if (hideOnPress) {
  ...
  render() {
    const { props } = this
    const {
      showCloseButton, closeSize, closeColor, closeStyle
    } = this.props
